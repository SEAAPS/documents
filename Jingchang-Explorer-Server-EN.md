...
* response
   ...
   2. If is block hash
      ...
      &emsp;&emsp;transNum|Number|number of transactions in the block|-|-
      &emsp;&emsp;parentHash|String|hash of the previous block|-|-
      &emsp;&emsp;totalCoins|String|total amount of SEAAPS|divided by 1000000 to get the actual SEAAPS amount|-
...
* request parameters
   ...
   w|String|true|-|-|wallet address
   c|String|false|-|-|transaction pair (You can pass no value. If you do n’t pass a value, it means to query the order of all types of transaction pairs. For example: SEAAPS-CNY or SEAAPS-cny, it must be in the form of the original sequential base-counter of the transaction pair. For example, it cannot be CNY -SEAAPS, otherwise the buying and selling relationship may be disordered. In addition, the transaction pair can only specify base or counter, such as SEAAPS- or -cny. For all transaction pairs, please refer to the [appendix](#appendix)）
...
* response
   ...
   &emsp;&emsp;fee|Number|transaction gas|unit is `SEAAPS`, decimal, minimum 0.00001|-
...
* request parameters
   ...
   c|String|false|-|-|token name（SWT does not require an issuer. The value is `SEAAPS_`. Other tokens need to be issued with the issuer in uppercase, like c=CNY_jGa9J9TkqtBcUoHe2zqhVFFbgUVED6o9or）
